...
def render():
    """Renders CSV results into README."""

    with open(SOURCE_HTML) as csvfile:
        results_html = [Result(name, round(int(req) / 15), *row) for name, req, *row in csv.reader(csvfile)]
    ...
    results = [
        Result(
            ...
            statistics.mean(map(float, [r1.lt50, r2.lt50, r3.lt50])),
            statistics.mean(map(float, [r1.lt75, r2.lt75, r3.lt75])),
            statistics.mean(map(float, [r1.lt90, r2.lt90, r3.lt90])),
            statistics.mean(map(float, [r1.lt_avg, r2.lt_avg, r3.lt_avg])),
                     r1.es + r2.es + r3.es,
                     r1.er + r2.er + r3.er,
                     r1.et + r2.et + r3.et,
        )
    ...
    # Render README
    render_template(README_TEMPLATE, README, **data)

    # Render pages
    render_template(PAGES_HOME_TEMPLATE, PAGES_HOME, **data)
    render_template(PAGES_RESULTS_TEMPLATE, PAGES_RESULTS, **data)
